# -*- coding: utf-8 -*-
"""
使用 multiprocessing 所提供的 Pool 函数 创建进程池
"""
import json
import random
import time
...
class ScrapeSpider:
    ...
    def run(self):
        ...
        for url in url_list:
            html_str = pool.apply_async(self.parse_url, args=(url, ))
            html_str = html_str.get()
            content_list = pool.apply_async(self.crawl_page, args=(html_str, ))
            content_list = content_list.get()
            pool.apply_async(self.save_to_file, args=(content_list, ))

        # html_str_list = []
        # for url in url_list:
        #     html_str = pool.apply_async(self.parse_url, args=(url,))
        #     html_str_list.append(html_str.get())
        # content_list_list = []
        # for html_str in html_str_list:
        #     content_list = pool.apply_async(self.crawl_page, args=(html_str,))
        #     content_list_list.append(content_list.get())
