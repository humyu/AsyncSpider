# -*- coding: utf-8 -*-
"""
ProcessPoolExecutor 使用 _base 模块的 map 函数
"""
import json
import random
import time
...
class ScrapeSpider:
    ...
    def run(self):
        with ProcessPoolExecutor(max_workers=5) as t:
            ...
            html_str_list = t.map(self.parse_url, url_list)
            content_list = t.map(self.crawl_page, html_str_list)
            t.map(self.save_to_file, content_list)
